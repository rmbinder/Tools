...
try {
    ...
    switch ($getMode) {
        case 'security_check':
            ...
            $page = PagePresenter::withHtmlIDAndHeadline('plg-tools-uninstall-html');
            $page->setTitle($title);
            $page->setHeadline($headline);

            $form = new FormPresenter('tools_uninstall__form', '../templates/uninstall.plugin.tools.tpl', '', $page, array(
                'type' => 'default',
                'method' => 'post',
                'setFocus' => false
            ));
        ...
        case 'uninst':
            ...
            if ($pPreferences->config['install']['access_role_id'] == 0 || $pPreferences->config['install']['menu_item_id'] == 0) {
                ...
                $result .= $gL10n->get('PLG_TOOLS_UNINST_NO_INST_IDS_FOUND');
            } else {
                $result_role = true;
                ...
                if (count($access_roles_prefs) === 1 && $access_roles_prefs[0] == $pPreferences->config['install']['access_role_id']) {
                    ...
                    $result .= $gL10n->get('PLG_TOOLS_UNINST_MENU_ITEM_NOT_DELETED');
                }
            }

            // Konfigurationsdaten löschen (aber nur in aktueller Organisation)
            $result_data = false;
            $result_db = false;
