...
/**
 ***********************************************************************************************
 * Uninstallation of the Admidio plugin Tools
 *
...
try
{
	...
	switch ($getMode)
	{
		case 'html':
			...
			$headline =$gL10n->get('PLG_TOOLS_UNINSTALLATION');
			
			$gNavigation->addUrl(CURRENT_URL, $headline);
			
			// create html page object
			$page = PagePresenter::withHtmlIDAndHeadline('plg-tools-uninstallation-html');
			$page->setTitle($title);
			$page->setHeadline($headline);
			
			$formUninstallation = new FormPresenter(
				'adm_preferences_form_uninstallation',
				'../templates/uninstallation.plugin.tools.tpl',
				SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . '/tools/system/uninstallation.php', array('mode' => 'uninst')),
				$page,
				array('class' => 'form-preferences')
			);
			
			$radioButtonEntries = array('0' => $gL10n->get('PLG_TOOLS_ACTORGONLY'), '1' => $gL10n->get('PLG_TOOLS_ALLORG') );
			
			$formUninstallation->addCheckbox('uninst_access_role', $gL10n->get('PLG_TOOLS_REMOVE_ACCESS_ROLE'));
			$formUninstallation->addRadioButton('uninst_access_role_org_select','',$radioButtonEntries, array('defaultValue' => '0', 'alertWarning' => $gL10n->get('PLG_TOOLS_UNINST_ACCESS_ROLE_ALERT')));
			
			$formUninstallation->addCheckbox('uninst_config_data', $gL10n->get('PLG_TOOLS_REMOVE_CONFIG_DATA'));
			$formUninstallation->addRadioButton('uninst_config_data_org_select','',$radioButtonEntries, array('defaultValue' => '0'));
			
			$formUninstallation->addCheckbox('uninst_menu_item', $gL10n->get('PLG_TOOLS_REMOVE_MENU_ITEM'), false, array('alertWarning' => $gL10n->get('PLG_TOOLS_UNINST_MENU_ITEM_ALERT')));
		...
		case 'uninst':
		    ...
		    $result = $gL10n->get('PLG_TOOLS_UNINST_STARTMESSAGE');
		    ...
		    if ($postUninstAccessRole)
		    {
		        ...
		        $result  .= ($result_role ? $gL10n->get('PLG_TOOLS_UNINST_ACCESS_ROLE_SUCCESS') : $gL10n->get('PLG_TOOLS_UNINST_ACCESS_ROLE_ERROR') );
		    }
		    ...
		    if ($postUninstMenuItem)
		    {
		        ...
		        if (count($roles) === 0)
		        {
		            ...
		            $result .= ($result_menu ? $gL10n->get('PLG_TOOLS_UNINST_MENU_ITEM_SUCCESS') : $gL10n->get('PLG_TOOLS_UNINST_MENU_ITEM_ERROR') );
		        }
		        else
		        {
		            $result .= $gL10n->get('PLG_TOOLS_ACCESS_ROLES_STILL_PRESENT');
		        }
		    }
		    ...
		    if ($postUninstConfigData)
		    {
		        ...
		        $result .= ($result_data ? $gL10n->get('PLG_TOOLS_UNINST_DATA_DELETE_SUCCESS') : $gL10n->get('PLG_TOOLS_UNINST_DATA_DELETE_ERROR') );
		        $result .= ($result_db ? $gL10n->get('PLG_TOOLS_UNINST_TABLE_DELETE_SUCCESS') : $gL10n->get('PLG_TOOLS_UNINST_TABLE_DELETE_ERROR') );
		    }
