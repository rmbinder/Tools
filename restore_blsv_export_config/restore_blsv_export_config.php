...
function isUserAuthorized($scriptName)
{
    ...
    {
        $gLogger->notice('RestoreBLSVExportConfig: Error with menu entry: Found rows: '. $menuStatement->rowCount() );
        $gLogger->notice('RestoreBLSVExportConfig: Error with menu entry: ScriptName: '. $scriptName);
        $gMessage->show($gL10n->get('PLG_RESTORE_BLSV_EXPORT_CONFIG_MENU_URL_ERROR', array($scriptName)), $gL10n->get('SYS_ERROR'));
    }
    else
    {
