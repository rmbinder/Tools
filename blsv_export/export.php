...
try {
    ...
    $postExportMode = admFuncVariableIsValid($_POST, 'export_mode', 'string', array(
        'defaultValue' => 'xlsx',
        'validValues' => array(
            'csv-ms',
            'csv-oo',
            ...
            if ($user->isMemberOfRole((int) $roleId)) {
                ...
                foreach ($columns as $data) {
                    ...
                    if (isset($data['usf_uuid'])) {
                        ...
                        if (($gProfileFields->getPropertyById($usf_id, 'usf_type') == 'DROPDOWN' || $gProfileFields->getPropertyById($usf_id, 'usf_type') == 'RADIO_BUTTON')) {
    ...
    if ($getMode == 'csv') {
        ...
        $keywords = array(
            'BLSV',
            $gL10n->get('PLG_BLSV_EXPORT_DATA_COMPARISON'),
            $gL10n->get('PLG_BLSV_EXPORT_MEMBERSHIP_REPORT')
        );
    ...
    } else // 'xml'
    {
        ...
        foreach ($rows as $row) {
            ...
            if (! isset($bsbnet_arr[$jahrgang]['fachverband'][$row[6]])) {
                $bsbnet_arr[$jahrgang]['fachverband'][$row[6]] = array(
                    'anzahlm' => 0,
                    'anzahlw' => 0
                );
            }
