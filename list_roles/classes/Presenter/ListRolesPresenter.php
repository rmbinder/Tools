...
class ListRolesPresenter extends PagePresenter
{
    ...
    /**
     * Show all roles of the organization in card view.
     * The roles must be read before with the method readData.
     ...
     *            UUID of the category for which the roles should be shown.
     * @param string $roleType
     *            The type of roles that should be shown within this page.
     *            0 - inactive roles
     *            1 - active roles
    ...
    public function createList(string $categoryUUID, string $roleType): void
    {
        ...
        foreach ($data as $row) {
            ...
            $templateRow['roleUrl'] = SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_MODULES . '/groups-roles/groups_roles.php', array(
                'mode' => 'edit',
            ...
            if (strlen($role->getValue('rol_description')) > 0) {
                ...
                if (strlen($roleDescription) > 50) {
                    ...
                    $roleDescription = substr($textPrev, 0, $maxPosPrev) . ' <span class="collapse" id="viewdetails-' . $row['rol_uuid'] . '">' . substr($roleDescription, $maxPosPrev) . '.
            ...
            if (strlen((string) $role->getValue('rol_cost')) > 0 || $role->getValue('rol_cost_period') > 0) {
                ...
                // Contributory period
                if (strlen((string) $role->getValue('rol_cost_period')) > 0 && $role->getValue('rol_cost_period') != 0) {
            ...
            $templateRow['actions'][] = array(
                'url' => SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_MODULES . '/messages/messages_write.php', array(
                    'role_uuid' => $row['rol_uuid']
                )),
                'icon' => 'bi bi-envelope',
            ...
            if ($gCurrentUser->hasRightViewProfiles($row['rol_id']) && ($row['num_members'] > 0 || $row['num_leader'] > 0)) {
                $templateRow['actions'][] = array(
                    'url' => SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_MODULES . '/groups-roles/groups_roles.php', array(
                        'mode' => 'export',
                        'role_uuid' => $row['rol_uuid']
                    )),
                    'icon' => 'bi bi-download',
            ...
            $templateRow['actions'][] = array(
                'url' => SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_MODULES . '/groups-roles/members_assignment.php', array(
                    'role_uuid' => $row['rol_uuid']
                )),
            ...
            $templateRow['actions'][] = array(
                'url' => SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_MODULES . '/groups-roles/lists_show.php', array(
                    'mode' => 'html',
                    'role_list' => $row['rol_uuid']
                )),
            ...
            if ($roleType == $this::ROLE_TYPE_INACTIVE && ! $role->getValue('rol_administrator')) {
                $templateRow['actions'][] = array(
                    'dataHref' => 'callUrlHideElement(\'role_' . $row['rol_uuid'] . '\', \'' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_MODULES . '/groups-roles/groups_roles.php', array(
                        'mode' => 'activate',
                        'role_uuid' => $row['rol_uuid']
                    )) . '\', \'' . $gCurrentSession->getCsrfToken() . '\')',
                    'dataMessage' => $gL10n->get('SYS_ACTIVATE_ROLE_DESC', array(
                        $row['rol_name']
                    )),
                    'icon' => 'bi bi-eye',
                ...
                $templateRow['actions'][] = array(
                    ...
                    'dataMessage' => $gL10n->get('SYS_DEACTIVATE_ROLE_DESC', array(
                        $row['rol_name']
                    )),
        ...
        $dataTables->disableColumnsSort(array(
            7
        ));
        $dataTables->setColumnsNotHideResponsive(array(
            7
        ));
    ...
    /**
     * Create content that is used on several pages and could be called in other methods.
     * It will
    ...
    protected function createHeader(string $categoryUUID, string $roleType): void
    {
        ...
        $form = new FormPresenter('adm_navbar_filter_form', 'sys-template-parts/form.filter.tpl', ADMIDIO_URL . FOLDER_PLUGINS . '/Tools/list_roles/list_roles.php', $this, array(
            'type' => 'navbar',
            'setFocus' => false
        ));

        $form->addSelectBoxForCategories('cat_uuid', $gL10n->get('SYS_CATEGORY'), $gDb, 'ROL', FormPresenter::SELECT_BOX_MODUS_FILTER, array(
            'defaultValue' => $categoryUUID
        ));
        if ($gCurrentUser->isAdministratorRoles()) {
            $form->addSelectBox('role_type', $gL10n->get('SYS_ROLE_TYPES'), array(
                0 => $gL10n->get('SYS_INACTIVE_GROUPS_ROLES'),
                1 => $gL10n->get('SYS_ACTIVE_GROUPS_ROLES'),
                2 => $gL10n->get('SYS_ROLES_CONFIRMATION_OF_PARTICIPATION')
            ), array(
